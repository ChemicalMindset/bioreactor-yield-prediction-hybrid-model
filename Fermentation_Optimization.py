...
def static_data():
    ...
    T_array = np.clip(T_array, 15.0, 50.0)  # type: ignore # Limit temperature to realistic range
    ...
    plt.title('Substrate effect with Inhibition (Ki = {Ki} g/L)')
    plt.legend()
    plt.grid(True)
    plt.show()
